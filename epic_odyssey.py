...
TITLE_ART = r"""
  ______     _                     ____   __   ___    _   _
 |  ____|   | |                   / __ \ / _| / _ \  | | | |
 | |__   ___| |__   ___  ___     | |  | | |_ | | | | | |_| |
 |  __| / __| '_ \ / _ \/ __|    | |  | |  _|| | | | |  _  |
 | |____\__ \ | | |  __/\__ \    | |__| | |  | |_| | | | | |
 |______|___/_| |_|\___||___/     \____/|_|   \___/  \_| |_/
"""
...
def animated_print(text: str, delay: float = ANIMATION_DELAY) -> None:
...
def animate_dots(label: str, steps: int = 3, delay: float = 0.25) -> None:
...
class Game:
    ...
    def new_game(self) -> None:
        ...
        self.player = self.build_player(name, archetype)
        self.market_state = self.build_market_state()
        self.game_over = False
        animate_dots("Forging charter")
        animated_print(wrap(
            f"Welcome, {name}. This is a long-form campaign tuned for around 10 hours if you play "
            "story and trade seriously. Save often."
        ))

    def choose_archetype(self) -> str:
        options = ["Blademaster", "Warden", "Arcanist"]
        print("\nChoose your charter class:")
        print("1) Blademaster (high attack)")
        print("2) Warden (high defense + hp)")
        print("3) Arcanist (high stamina + elixir)")
        pick = ask_int("Class: ", 1, 3)
        archetype = options[pick - 1]
        print(CLASS_ART[archetype])
        return archetype

    def build_player(self, name: str, archetype: str) -> Player:
        player = Player(name=name, archetype=archetype)
        if archetype == "Blademaster":
            player.attack += 3
            player.stamina += 4
            player.max_stamina += 4
        elif archetype == "Warden":
            player.defense += 3
            player.hp += 24
            player.max_hp += 24
        elif archetype == "Arcanist":
            player.max_stamina += 14
            player.stamina += 14
            player.elixirs += 1
        return player
    ...
    def render_hud(self) -> None:
        ...
        print(f"\n--- Day {p.day} | {p.name} the {p.title} ({p.archetype}) | Lv{p.level} ---")
        print(
    ...
    def chapter_banner(self, chapter: int) -> None:
        marks = "=" * 44
        print(f"\n{marks}")
        print(f" CHAPTER {chapter}: {CHAPTER_TITLES.get(chapter, 'Echoes of Aetherfall')}")
        print(marks)
